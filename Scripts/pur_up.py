...
def fetch_purchase_data():
    try:
        cursor.execute("SELECT Invoice, Sold_to, Item_Name, Quantity, Price, Date FROM Purchase")
...
# Function to display purchase data in the Treeview
...
# Function to update the selected purchase record
def update_purchase():
    selected_item = tree.selection()
    if not selected_item:
    ...
    try:
        cursor.execute(
            """
            UPDATE Purchase
            SET Sold_to = ?, Item_Name = ?, Quantity = ?, Price = ?, Date = ?
            WHERE Invoice = ?
            """,
...
def refresh_data():
    ...
    if data:
        display_purchase_data(data)
    else:
        messagebox.showinfo("Info", "No purchase data found.")
...
def filter_by_date():
    ...
    try:
        cursor.execute(
            """
            SELECT Invoice, Sold_to, Item_Name, Quantity, Price, Date
            FROM Purchase
...
def search_by_invoice():
    ...
    try:
        cursor.execute("SELECT Invoice, Sold_to, Item_Name, Quantity, Price, Date FROM Purchase WHERE Invoice = ?", (invoice,))
        data = cursor.fetchall()
        if data:
            display_purchase_data(data)
        else:
...
# Heading
tk.Label(main_frame, text="Update Purchase", font=("Arial", 18, "bold"), bg="white").grid(row=0, column=0, columnspan=4, pady=10)
...
# Purchase date
tk.Label(main_frame, text="Purchase Date:", bg="white").grid(row=8, column=0, padx=10, pady=10, sticky="e")
entry_date = DateEntry(main_frame, date_pattern="yyyy-mm-dd", width=30)
entry_date.grid(row=8, column=1, padx=10, pady=10, sticky="w")

# Buttons
btn_update = tk.Button(main_frame, text="Update Purchase", command=update_purchase)
